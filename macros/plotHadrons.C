...
//
//  
void plotHadrons(const string& file="", int iStart=0, int iStop=10000) {

  if ( file=="" ) {
    cout << "\n usage: .x plotHadrons.C(\"<conex-file-name(s)>\", startShower, endShower) \n"
	 << endl;
    return;
  }
  ...
  const int maxX = 5000;
  float X[maxX], H[maxX], D[maxX], N[maxX],eN[maxX],Hadrons[maxX];
  ...
  Shower->SetBranchAddress("Hadrons",Hadrons);       // Hadrons(X)      
  ...
  if ( tmp ) delete tmp;
  TProfile * allmu  =  new TProfile("allmu","hadrons",maxX,0,2010," ");
  ...
  for (int i=iStart; i<=(int)TMath::Min((double)Shower->GetEntries(), (double)iStop); ++i) {
    ...
    //    if(fitpars[0]>0){         // energy
    if(fitpars[1]<10){         // angle

    for (int j=0;j<nX; j++) {
      allmu->Fill(X[j],Hadrons[j]);	
    }
  ...
  allmu->GetYaxis()->SetTitle("number of hadrons");
